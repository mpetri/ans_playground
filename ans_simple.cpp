...
static inline RansState RansEncRenorm(
...
static inline void RansEncSymbolInit(
    RansEncSymbol* s, uint32_t start, uint32_t freq, uint32_t scale_bits)
{
    ...
    if (freq < 2) {
        ...
        //     = floor(x - x/(2^32))
        //     = x - 1 if 1 <= x < 2^32
        // and we know that x>0 (x=0 is never in a valid normalization
        // interval).
        //
...
static inline void RansDecSymbolInit(
...
static inline void RansEncPutSymbol(
    RansState* r, uint8_t** pptr, RansEncSymbol const* sym)
{
    ...
    //    *r = ((x / sym->freq) << sym->scale_bits) + (x % sym->freq) +
    //    sym->start;
    //    return;
    ...
    *r = x + sym->bias + q * sym->cmpl_freq;
    ...
    //    assert(((x / sym->freq) << sym->scale_bits) + (x % sym->freq) +
    //    sym->start == *r);
}

// Equivalent to RansDecAdvance that takes a symbol.
static inline void RansDecAdvanceSymbol(
    RansState* r, uint8_t** pptr, RansDecSymbol const* sym, uint32_t scale_bits)
{
...
static inline void RansDecAdvanceStep(
...
static inline void RansDecAdvanceSymbolStep(
    RansState* r, RansDecSymbol const* sym, uint32_t scale_bits)
{
...
{
    int F1[256 + MAGIC] = { 0 }, F2[256 + MAGIC] = { 0 },
                 F3[256 + MAGIC] = { 0 };
    int i;
...
static void hist8(unsigned char* in, unsigned int in_size, int F0[256])
{
    int F1[256 + MAGIC] = { 0 }, F2[256 + MAGIC] = { 0 },
                 F3[256 + MAGIC] = { 0 };
    int F4[256 + MAGIC] = { 0 }, F5[256 + MAGIC] = { 0 },
                 F6[256 + MAGIC] = { 0 }, F7[256 + MAGIC] = { 0 };
    int i, i8 = in_size & ~7;
    for (i = 0; i < i8; i += 8) {
        F0[in[i + 0]]++;
        F1[in[i + 1]]++;
...
{
    int tabsz = order == 0 ? 257 * 3 + 4 + NX * 4 + NX * 4
    ...
    unsigned char* out, unsigned int* out_size)
{
    ...
    fsum++; // not needed, but can't remove without removing assert x<TOTFREQ
            // (in old code)
    ...
    unsigned char* out, unsigned int* out_size)
{
    ...
    for (i = 0; i < out_end; i += NX) {
        ...
        Rv1 = _mm256_add_epi32(
            _mm256_mullo_epi32(_mm256_srli_epi32(Rv1, TF_SHIFT), fv1), bv1);
        Rv2 = _mm256_add_epi32(
            _mm256_mullo_epi32(_mm256_srli_epi32(Rv2, TF_SHIFT), fv2), bv2);
        ...
        *(uint64_t*)&out[i + 8] = _mm256_extract_epi64(sv1, 2);
        ...
        // c =  R[z] < RANS_BYTE_L;
        __m256i renorm_mask1
            = _mm256_xor_si256(Rv1, _mm256_set1_epi32(0x80000000));
        __m256i renorm_mask2
            = _mm256_xor_si256(Rv2, _mm256_set1_epi32(0x80000000));
        renorm_mask1 = _mm256_cmpgt_epi32(
            _mm256_set1_epi32(RANS_BYTE_L - 0x80000000), renorm_mask1);
        renorm_mask2 = _mm256_cmpgt_epi32(
            _mm256_set1_epi32(RANS_BYTE_L - 0x80000000), renorm_mask2);
...
#ifdef UNUSED
static void hist1_1(
    unsigned char* in, unsigned int in_size, int F0[256][256], int T0[256])
{
...
static void hist1_4(
    unsigned char* in, unsigned int in_size, int F0[256][256], int* T0)
{
    int T1[256 + MAGIC] = { 0 }, T2[256 + MAGIC] = { 0 },
                 T3[256 + MAGIC] = { 0 };
    ...
    unsigned char last_0 = 0, last_1 = in1[-1], last_2 = in2[-1],
                  last_3 = in3[-1];
...
//#define BRANCHLESS
        ...
        uint32_t j4[]
            = { !(R[0] & ~(RANS_BYTE_L - 1)), !(R[1] & ~(RANS_BYTE_L - 1)),
                !(R[2] & ~(RANS_BYTE_L - 1)), !(R[3] & ~(RANS_BYTE_L - 1)) };
...
unsigned char* rans_uncompress_O1sfb_4x16(unsigned char* in,
    unsigned int in_size, unsigned char* out, unsigned int* out_size)
{
    /* Load in the static tables */
    unsigned char* cp = in + 4;
    int i, j = -999, x, y, out_sz, rle_i, rle_j;
    sb_t sfb[256][TOTFREQ_O1 + 32];
    // uint16_t for ssym sometimes works faster, but considter 8-bit if cache is
    // tight
...
{
    return order ? rans_compress_O1_4x16(in, in_size, out, out_size)
                 : rans_compress_O0_4x16(in, in_size, out, out_size);
}

unsigned char* rans_compress_4x16(
    unsigned char* in, unsigned int in_size, unsigned int* out_size, int order)
{
...
unsigned char* rans_uncompress_to_4x16(unsigned char* in, unsigned int in_size,
    unsigned char* out, unsigned int* out_size, int order)
{
    return order ? rans_uncompress_O1sfb_4x16(in, in_size, out, out_size)
                 //? rans_uncompress_O1c_4x16(in, in_size, out, out_size)
                 : rans_uncompress_O0_4x16(in, in_size, out, out_size);
}

unsigned char* rans_uncompress_4x16(
    unsigned char* in, unsigned int in_size, unsigned int* out_size, int order)
{
...
#define NTRIALS 10
        ...
        while (trials--) {
            ...
            for (i = 0; i < nb; i++) {
                ...
                bc[i].blk = rans_compress_to_4x16(
                    b[i].blk, b[i].sz, bc[i].blk, &csz, order);
                assert(csz <= bc[i].sz);
                out_sz += 5 + csz;
            }
            ...
            // Warmup
            for (i = 0; i < nb; i++)
                memset(bu[i].blk, 0, BLK_SIZE);

            gettimeofday(&tv3, NULL);

            for (i = 0; i < nb; i++)
                bu[i].blk = rans_uncompress_to_4x16(
                    bc[i].blk, bc[i].sz, bu[i].blk, &bu[i].sz, order);

            gettimeofday(&tv4, NULL);

            for (i = 0; i < nb; i++) {
                if (b[i].sz != bu[i].sz
                    || memcmp(b[i].blk, bu[i].blk, b[i].sz)) {
                    int z;
                    for (z = 0; z < b[i].sz; z++)
            ...
            fprintf(stderr,
                "%5.1f MB/s enc, %5.1f MB/s dec\t %lld bytes -> %lld bytes\n",
    ...
    if (decode) {
        ...
        for (;;) {
            ...
            out = rans_compress_4x16(
                in_buf, in_size, &out_size, order && in_size >= 4);
    ...
    fprintf(stderr, "Took %ld microseconds, %5.1f MB/s\n",
        (long)(tv2.tv_sec - tv1.tv_sec) * 1000000 + tv2.tv_usec - tv1.tv_usec,
        (double)bytes / ((long)(tv2.tv_sec - tv1.tv_sec) * 1000000 + tv2.tv_usec
                            - tv1.tv_usec));
    return 0;
}
#endif
